

// import React from 'react';
// import { Link } from 'react-router-dom';
// import { FaSignOutAlt } from 'react-icons/fa'; // Correct icon import

// const Sidebar = ({ data }) => {
//   return (
//     <div className='bg-zinc-800 p-4 rounded flex flex-col items-between justify-between h-[100%]'>
//       <div className='flex items-center flex-col justify-center'>
//         <img src={data.avatar} className='h-[12vh]' alt='Avatar' />
//         <p className='mt-3 text-xl text-zinc-100 font-semibold'>
//           {data.username}
//         </p>
//         <p className='mt-1 text-normal text-zinc-300'>{data.email}</p>
//         <div className='w-full mt-4 h-[1px] bg-zinc-500 hidden lg:block'></div>
//       </div>
      
//       <div className='w-full flex-col items-center justify-center hidden lg:flex'>
//         <Link
//           to="/profile"
//           className='text-zinc-100 font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-900 rounded transition'
//         >
//           Favourites
//         </Link>
//         <Link
//           to="/profile/orderHistory"
//           className='text-zinc-100 font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-900 rounded transition'
//         >
//           Book History
//         </Link>
//         <Link
//           to="/profile/settings"
//           className='text-zinc-100 font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-900 rounded transition'
//         >
//           Settings
//         </Link>
//       </div>
      
//       <button className='bg-zinc-900 w-3/6 lg:w-full mt-4 lg:mt-0 text-white font-semibold flex items-center justify-center py-2 rounded hover:bg-white hover:text-zinc-900 transition-all duration-300'>
//         Log Out 
//         <FaSignOutAlt className='ms-4' />
//       </button>
//     </div>
//   );
// };

// export default Sidebar;

//new

import React from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { FaSignOutAlt } from 'react-icons/fa';
import { useDispatch,useSelector } from 'react-redux';
import { authActions } from '../../store/auth';

const Sidebar = ({ data }) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const role =useSelector((state) => state.auth.role);
  const handleLogout = () => {
    // Clear user data from localStorage or your chosen storage
    localStorage.removeItem('token');
    localStorage.removeItem('id');
    localStorage.clear("role");

    // Redirect to home page
    navigate('/');
    
    // Optionally, you can also dispatch a logout action if you are using a global state management like Redux
  };

  return (
    <div className='bg-zinc-400 p-4 rounded flex flex-col items-between justify-between h-[100vh]'>
      <div className='flex items-center flex-col justify-center'>
        <img src={data.avatar} className='h-[12vh]' alt='Avatar' />
        <p className='mt-3 text-3xl text-zinc-900 font-semibold'>
          {data.username}
        </p>
        <p className='mt-1 text-base text-zinc-800'>{data.email}</p>
        <div className='w-full mt-4 h-[1px] bg-zinc-500 hidden lg:block'></div>
      </div>
      
     {role==="user" && ( <div className='w-full flex-col items-center justify-center hidden lg:flex'>
        <Link
          to="/profile"
          className='text-zinc-900  text-xl font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-500 rounded transition'
        >
          Favourites
        </Link>
        <Link
          to="/profile/orderHistory"
          className='text-zinc-900  text-xl font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-500 rounded transition'
        >
          Book History
        </Link>
        <Link
          to="/profile/settings"
          className='text-zinc-900 text-xl font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-500 rounded transition'
        >
          Settings
        </Link>
      </div>)}

      {role ==="admin" && ( <div className='w-full flex-col items-center justify-center hidden lg:flex'>
        <Link
          to="/profile"
          className='text-zinc-900  text-xl font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-500 rounded transition'
        >
          All Order 
        </Link>
        <Link
          to="/profile/add-book"
          className='text-zinc-900  text-xl font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-500 rounded transition'
        >
          Add Book 
        </Link>
       
      </div>)}
      
      <button
        onClick={handleLogout}
        className='bg-zinc-900 w-3/6 lg:w-full mt-4 lg:mt-0 text-white font-semibold flex items-center justify-center py-2 rounded hover:bg-white hover:text-zinc-900 transition-all duration-300'
      >
        Log Out 
        <FaSignOutAlt className='ms-4' />
      </button>
    </div>
  );
};

export default Sidebar;









